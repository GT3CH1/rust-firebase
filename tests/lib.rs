...
#[test]
fn builds_auth_url() {
    let f = Firebase::authed("https://db.rifebass.com/", "deadbeaf")
        .ok()
        .unwrap();
...
fn test_ops() {
    ...
    let correct =
        Url::parse("https://db.fe//lol.json?limitToFirst=4&endAt=13&equalTo=8&shallow=false")
            .ok()
            .unwrap();
...
#[test]
fn test_auth_ops() {
    let f = Firebase::authed("https://db.fe/", "key")
        .ok()
        .expect("url err")
        .at("lol")
        .ok()
    ...
    let correct = Url::parse("https://db.fe/lol.json?auth=key&orderBy=pts&limitToLast=5&startAt=8")
        .ok()
        .unwrap();
    let generated = Url::parse(req.get_url()).ok().unwrap();
